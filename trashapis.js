...
function cycles(postcode, housenumber, country, callback) {
    if (country !== "NL") {
    ...
    request.post({
        url: 'https://afvalkalender.cyclusnv.nl/ajax/bag/postcode/huisnummer',
        json: true,
        headers: {
    ...
    }, function (err, res, bags) {
        if (!err && res.statusCode == 200 && bags.length > 0) {
            var bagId = bags[0]['bag_identificatie'];
            request.get(`https://afvalkalender.cyclusnv.nl/ical/${bagId}`, function (err, res, body) {
                if (!err && res.statusCode == 200) {
                    const dates = {};
                    ...
                    for (let i in entries) {
                        ...
                        if (entry.description === 'Groente-, fruit- en tuinafval') {
                            ...
                            console.log(`Unsupported type found in cycles ical with description ${entry.description}`);
                        }
                    }
