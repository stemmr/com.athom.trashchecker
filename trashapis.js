...
function afvalwijzerArnhem(postcode, housenumber, country, callback) {
    ...
    request(url, function(err, res, body) {
        if (!err && res.statusCode == 200) {
            ...
            var $ = cheerio.load(res.body);
            $('td.item').each((i, elem) => {
                if ($(elem).attr('data-type') == 'pickup') {
                    //console.log($(elem).attr('data-categories'));
                    var garbageItems = $(elem).attr('data-categories').split(',');
                    garbageItems.forEach(function(item) {
                        //console.log($(elem).text());
                        //console.log(item);
                        var datum = ('0' + $(elem).text()).slice(-2) + ' ' + $('span[class=monthName]').text().trim(' ');
                        //console.log(dateFormat(datum));
                        var dateStr = dateFormat(datum);
                        switch (item) {
                            case 'gft':
                                if (!fDates.GFT) fDates.GFT = [];
                                fDates.GFT.push(dateStr);
...
function dateFormat(date) {
    ...
    var monthNum = months.indexOf(ad[1]) + 1;
    if (monthNum > 0) {
        var monthString = (monthNum).toString();
        if (monthString.length === 1) {
            monthString = '0' + monthString;
        }
        fullString += monthString + '-' + ad[2];
    } else {
